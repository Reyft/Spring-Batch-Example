...
public class PersonJdbcWriter1 implements ItemWriter<Personne> {

    private JdbcTemplate jdbcTemplate = new JdbcTemplate();

    private static final String REQUEST_INSERT_PERSONNE = "insert into monsieur (nom,prenom,privateId,createdDate, updatedDate) values (?,?,?,?,?)";
    private static final String REQUEST_UPDATE_PERSONNE = "update monsieur set nom=?, prenom=?, occurence=?, updatedDate=? where privateId=?";
    private static final String REQUEST_OCCURENCE = "select occurence from monsieur where privateId=?";
    ...
    public void write(List<? extends Personne> items) throws Exception {
        for (Personne personne : items){
            ...
            try{
                occurence = jdbcTemplate.queryForObject(REQUEST_OCCURENCE, new Object[] {personne.getId()}, Integer.class);
